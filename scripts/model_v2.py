...
構成:
    logit(p) = logit(Log5(ピタゴラス勝率)) + log(ホーム補正) + 0.25 × 先発の質の差

確信度で絞ると的中率が上がることも確認済み(全期間):
    60%以上 → 60.8% (週19.6試合)
    65%以上 → 68.5% (週6.3試合)  ← 「本命」の基準に採用
    70%以上 → 68.0% (週1.7試合)
"""
...
SP_WEIGHT = 0.25        # 先発の重み(バックテストで決定)
MIN_GAMES = 20          # これ未満の消化試合数だと成績が不安定
CONFIDENT_THRESHOLD = 65  # 「本命」= 実測的中68.5% (週6.3試合)
WATCH_THRESHOLD = 62      # 「注目」= 実測的中66.3% (週11.6試合)

# バックテストで実測した、確信度帯ごとの的中率(全期間)
BACKTEST_HIT_RATES = {60: 60.8, 62: 66.3, 65: 68.5, 68: 62.7, 70: 68.0}
